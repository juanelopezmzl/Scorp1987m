...
module.exports = {
     ...
     * @param {number} limit 
     * @param {number} page 
     * @param {string} telegram_name
     * 
     */
    async getLogAsync(limit = null, page = null, telegram_name = null){
        if(!limit) limit = 20;
        if(!page) page = 1;
        const where = (telegram_name) ? `WHERE LOWER(u.telegram_name)=LOWER('${telegram_name}')` : '';
        const sql = `SELECT l.log_id,l.logging_date,u.telegram_name,u.telegram_user_name,l.activity,b.telegram_name AS by_telegram_name,b.telegram_user_name AS by_telegram_user_name FROM trackings l LEFT JOIN users u ON l.telegram_user_id=u.telegram_user_id LEFT JOIN users b ON l.by_telegram_user_id=b.telegram_user_id ${where} ORDER BY l.logging_date DESC OFFSET ${limit * (page-1)} ROWS FETCH NEXT ${limit} ROWS ONLY;`;
